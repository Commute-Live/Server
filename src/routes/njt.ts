...
const parseDirection = (_mode: CoreMode, value: string | undefined) => {
...
const providerIdForMode = (mode: CoreMode) => (mode === "rail" ? "njt-rail" : "njt-bus");
...
export function registerNjtRoutes(app: Hono, deps: dependency) {
    app.get("/njt/stations", async (c) => {
        const mode = parseCoreMode(c.req.query("mode") ?? "");
        if (!mode) return c.json({ error: "mode is required (rail|bus)" }, 400);
    ...
    app.get("/njt/stations/:mode/lines", async (c) => {
    ...
    app.get("/njt/stations/:mode/:stopId/lines", async (c) => {
        const mode = parseCoreMode(c.req.param("mode") ?? "");
        if (!mode) return c.json({ error: "Invalid mode" }, 400);
    ...
    app.get("/njt/stations/:mode/:line/stopId", async (c) => {
        const mode = parseCoreMode(c.req.param("mode") ?? "");
        if (!mode) return c.json({ error: "Invalid mode" }, 400);
    ...
    app.get("/njt/stations/:mode/:stopId/arrivals", async (c) => {
        const mode = parseCoreMode(c.req.param("mode") ?? "");
        if (!mode) return c.json({ error: "Invalid mode" }, 400);
        ...
        let providerStops = Array.from(new Set(station.childStopIds.map((v) => v.trim()).filter((v) => v.length > 0)));
